...
isomorphism $\theta_{C,A} : C_1 \longrightarrow A^1$.
...
sea $M$ subespacio con un $g\in M^\ast$, verificando:
...
Se verifica:
 ...
 1. $\forall x \in X: J_X(x)$ es lineal y de módulo $1$.
...
tal que $f(M) = \{0\}$ y $f(x_0) = d(x_0,\overline{M}) = d(x_0,M)$.
...
**** Teorema de la aplicación abierta
Sean $X,Y$ espacios de Banach con $T \in L(X,Y)$, lineal,
continua, y sobreyectiva. Entonces, $T$ es abierta, y 
...
lipschitziana por la condición. Por el [[*Teorema de los isomorfismos de Banach][teorema de los isomorfismos
de Banach]], es isomorfismo topológico.
...
cuando $f$ es continua, $Graf(f)$ es cerrada.
...
**** Corolario a Banach-Steinhaus para Banach
...
**** Corolario a Banach-Steinhaus para normados

...
**** Base de una torre de inclusiones
Sea $E \supset F \supset K$ una torre de inclusiones. Sean $\{u_i\}$ una base de $E$
sobre $F$ y $\{v_i\}$ una base de $F$ sobre $K$.
...
**** Corolario al Teorema del grado: finitud
**** Corolario al Teorema del grado: torres de cuerpos
**** Corolario al Teorema del grado: extensiones primas
...
**** Característica en dominios de integridad
Si $A$ es dominio de integridad, $car(A)$ es primo o $0$.

***** Demostración
Trivialmente desde el homomorfismo unital.

**** Caracterización de la característica
...
y es el menor, todo el resto de elementos del núcleo deben ser múltiplos,
porque si no lo fueran, podríamos crear un menor con Bezout.
...
**** Anillo primo
Llamamos subanillo primo de $A$ a la intersección de todos los subanillos
de $A$.

**** Clasificación de anillos primos
El subanillo primo de un $A$ es isomorfo a $\mathbb{Z}$ si $car(A) = 0$ y
a $\mathbb{Z}/n\mathbb{Z}$ si $car(A) = n \neq 0$. Cuando $A$ es dominio de integridad,
o $car(A) = 0$, o $car(A)$ es primo.
...
**** Clasificación de subcuerpos primos
...
**** Propiedades de subanillos y subcuerpos generados
...
**** Subcuerpo compuesto
Dados $L \supset E,F \supset K$, definimos el subcuerpo compuesto
$EF = E(F) = F(E)$.
...
*Polinomio irreducible*. Dado $F/K$ con $\alpha \in F$ algebraico. Existe un 
único polinomio irreducible del que $\alpha$ es raíz salvo asociados, 
llamado $Irr(\alpha)$.
...
**** 4.1.6. Los endomorfismos algebraicos son automorfismos
Sea $F/K$ una extensión algebraica y $\sigma : F \longrightarrow F$ un homomorfismo
sobre $K$. Entonces $\sigma$ es un automorfismo
...
que $\sigma(F) \subset F$, y por ser algebraico, todo endomorfismo es automorfismo.
...
**** Existencia y unicidad de la clausura normal
Para toda extensión algebraica existe una clausura normal y es
única salvo isomorfismos.
...
algebraica $F/K$ con una raíz de $f$, descompone en factores lineales.
...
Un elemento $u$ es separable en $K$ si $Irr(u,K)$ no tiene raíces
múltiples.

**** 5.3.2. Extensiones separables
Una extensión $F/K$ es *separable* si todos sus elementos lo son.
...
\[ [F : K]_s = \#\{\sigma : F/K \longrightarrow \overline{K}/K \text{ sobre } K\}\]
...
Las extensiones separables cumplen:
 ...
 2. Sea $E/K$ separable y $H/K$ extensión, entonces $EH/H$ es 
    separable.
...
intermedios $\{ F \mid K \subset F \subset E\}$ es finito. Si una extensión es finita

**** 5.3.14. Endomorfismo de Frobenius
Sea $K$ de característica de $p$. Llamamos *endomorfismo de Frobenius* 
a $\phi(u) = u^p$.

**** 5.3.15. Caracterización de cuerpos perfectos
...
de escalares elevados desde $F$ forma un espacio vectorial de dimensión
$|S|$.

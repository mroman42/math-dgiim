...
***** Demostración
Sean $H: f \simeq g$, veamos que $F\circ H: Ff \simeq Fg$. Tenemos que:

  - $F\circ H$ continua
  - $F\circ H(t,0) = F\circ f(t)$
  - $F\circ H(t,1) = F\circ g(t)$
  - $F \circ H(0,s) = F \circ H(1,s) = y$
...
Usaremos teoría de recubridores que se probará luego.
...
Una *equivalencia homotópica* es un par de aplicaciones continuas $F,G$
cumpliendo $F \circ G \simeq G \circ F \simeq Id$.
...
Sea $(Y,p)$ recubridor con $x_0\in X$ y $y \in p^{-1}(x_0)$. Sea $f : [0,1]\longrightarrow X$ arco continuo
...
**** Levntamiento de de aplicaciones
...
a $\check{e}$ como elemento normal y a $p$ como homomorfismo de grupos.

...
**** Extensiones separables
 Una *elemento separable* $u$ es aquel cumpliendo que $\operatorname{Irr}(u,K)$ no tiene raíces 
...
**** Z es objeto inicial
El anillo $\mathbb{Z}$ es inicial en =CRing=.
...
**** TODO Preservación de ideales primos y maximales
*** Teorema de Krull
...
*** TODO Ideales contraídos y extendidos
...
**** Teorema de la aplicación abierta
Sean $X,Y$ espacios de Banach con $T: X \longrightarrow Y$ lineal, continua y sobreyectiva.
Entonces, $T$ es abierta.

**** Corolario de los isomorfismos de Banach
Sean $X,Y$ espacios de Banach con $T : X \longrightarrow Y$ biyección lineal continua.
